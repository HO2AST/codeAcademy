...
<body>
	...
	<%-- =========================================================== --%>
	<div class="row">
		<h2 class="community_board">자료게시판</h2>
		<div class="col-md-12">
					...
					<c:choose>
						<c:when test="${pageInfoMap.count gt 0 }">
							<c:forEach var="board" items="${boardList }">
								<tr onclick="location.href='communityContent.do?num=${board.num }&pageNum=${pageNum }&prevPage=${curPage }';">
									<td style="font-size: 12px;">${board.num }</td>
									<td>${board.category}</td>
									<td id="title">
										<c:if test="${board.reLev gt 0 }">
											<c:set var="level" value="${board.reLev * 10 }" />
											<img src="imgs/level.gif" height="13" width="${level}">
		...
		<div class="col-md-12">
			...
			<c:if test="${pageInfoMap.startPage gt pageInfoMap.pageBlock }">
				<a href="downloadForm.do?pageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock + 4}&search=${search}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${pageInfoMap.startPage}" end="${pageInfoMap.endPage}" step="1">
				<a href="downloadForm.do?pageNum=${i}&search=${search}">
				<c:choose>
			...
			<c:if test="${pageInfoMap.endPage lt pageInfoMap.pageCount}">
				<a href="downloadForm.do?pageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock + 4}&search=${search}">[다음]</a>
			</c:if>
		</div>
		
		<form action="downloadForm.do" method="get">
			<div id="board_search">
				<c:choose>
					<c:when test="${!empty id}">
						...
						<button type="button" value="글쓰기" id="write_btn" onclick="location.href='communityWriteForm.do?prevPage=${curPage}';" >글쓰기</button>
					</c:when>
					<c:otherwise>
						<input type="text" placeholder="제목을 입력하세요" name="search" value="${search}" class="input_box"> 
						<input type="submit" value="제목검색" id="search_btn">
